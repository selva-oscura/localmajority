...
class Issue extends Component {
  ...
  render() {
    ...
    return issue
        ...
        {subIssues.map(subIssue => (
          <Aux key={subIssue.title}>
            ...
            {articles.map(article => {
              ...
              return (
                article.tags.includes(subIssue.slug)
                ? (
                  <ArticleLink
      ...
      <div className="Issue">
        <h2>{this.props.match.params.slug} -- HEY!  COMPARE THIS WITH AN ISSUE TAG</h2>
        {articles.map((article, i) => (
          <Aux key={i}>
            <Link to={`/articles/${article.slug}`}>
              <p className="row">
                <span className="col-sm-4">{article.articleType}</span>
                <span className="col-sm-8">{article.title} -- {article.tags.join(', ')}}</span>
              </p>
            </Link>
          </Aux>
        ))}
